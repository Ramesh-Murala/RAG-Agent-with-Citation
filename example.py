...
def build_fake_response(question: str, context_block: str):
    ...
    if "pto" in q or "paid time off" in q or "roll over" in q:
        return {
            ...
            "citations": [
                citation_for("Full-time employees accrue 15 days of paid time off (PTO) per year.", chunk_ids[0]),
                citation_for("Up to 5 unused PTO days may be rolled over into the next calendar year; anything beyond that is forfeited on December 31st.", chunk_ids[0]),
            ],
...
def main() -> None:
    ...
    if use_fake_llm:
        print("Running in local offline mode with a fake LLM client (no API key required).")
